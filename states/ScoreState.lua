...
function ScoreState:render()
    ...
    if self.score >= 3 then
        love.graphics.draw(self.gold, VIRTUAL_WIDTH/2 - 20, 115)
    elseif self.score >= 2 then
        love.graphics.draw(self.silver, VIRTUAL_WIDTH/2 - 20, 115)
    elseif self.score >= 1 then
        love.graphics.draw(self.bronze, VIRTUAL_WIDTH/2 - 20, 115)
    end
